...
def get_required_sma_lengths(strategy_type: Any) -> list[int]: return call_first_available(strategy_type, ["get_required_sma_lengths"])
def get_required_avg_range_lookbacks(strategy_type: Any) -> list[int]: return call_first_available(strategy_type, ["get_required_avg_range_lookbacks"])
def get_required_momentum_lookbacks(strategy_type: Any) -> list[int]: return call_first_available(strategy_type, ["get_required_momentum_lookbacks"])
def build_sanity_check_strategy(strategy_type: Any): return call_first_available(strategy_type, ["build_sanity_check_strategy", "get_sanity_check_strategy", "build_default_strategy"])
def run_family_filter_combination_sweep(strategy_type: Any, data: pd.DataFrame, cfg: EngineConfig, max_workers: int) -> pd.DataFrame: return call_first_available(strategy_type, ["run_filter_combination_sweep", "run_family_filter_combination_sweep"], data=data, cfg=cfg, max_workers=max_workers)
def get_promotion_gate_config(strategy_type: Any) -> dict[str, Any]: return call_first_available(strategy_type, ["get_promotion_gate_config"])
def get_refinement_grid_for_candidate(strategy_type: Any, candidate_row: dict[str, Any]) -> dict[str, list[Any]]: return call_first_available(strategy_type, ["get_refinement_grid_for_candidate", "build_refinement_grid_for_candidate"], candidate_row)
def run_top_combo_refinement(strategy_type: Any, data: pd.DataFrame, cfg: EngineConfig, candidate_row: dict[str, Any], max_workers: int) -> pd.DataFrame: return call_first_available(strategy_type, ["run_top_combo_refinement", "run_refinement_for_candidate"], data=data, cfg=cfg, candidate_row=candidate_row, max_workers=max_workers)
...
def apply_promotion_gate(combo_results_df: pd.DataFrame, promotion_gate: dict[str, Any]) -> pd.DataFrame:
    ...
    min_pf, min_avg_trade = float(promotion_gate.get("min_profit_factor", 0.0)), float(promotion_gate.get("min_average_trade", float("-inf")))
    require_positive_net_pnl, min_trades, min_trades_per_year = bool(promotion_gate.get("require_positive_net_pnl", False)), int(promotion_gate.get("min_trades", 0)), float(promotion_gate.get("min_trades_per_year", 0.0))
    ...
    if dedup_cols: promoted = promoted.drop_duplicates(subset=dedup_cols).reset_index(drop=True)
    return promoted
...
def build_family_leaderboard(summary_df: pd.DataFrame) -> pd.DataFrame:
    ...
    leaderboard["quality_flag"] = leaderboard.apply(lambda r: select_best(r, "best_refined_quality_flag", "best_combo_quality_flag"), axis=1)
    leaderboard["is_trades"] = leaderboard.apply(lambda r: select_best(r, "best_refined_is_trades", "best_combo_is_trades"), axis=1)
    leaderboard["oos_trades"] = leaderboard.apply(lambda r: select_best(r, "best_refined_oos_trades", "best_combo_oos_trades"), axis=1)
    leaderboard["is_pf"] = leaderboard.apply(lambda r: select_best(r, "best_refined_is_pf", "best_combo_is_pf"), axis=1)
    leaderboard["oos_pf"] = leaderboard.apply(lambda r: select_best(r, "best_refined_oos_pf", "best_combo_oos_pf"), axis=1)
    leaderboard["recent_12m_trades"] = leaderboard.apply(lambda r: select_best(r, "best_refined_recent_12m_trades", "best_combo_recent_12m_trades"), axis=1)
    leaderboard["recent_12m_pf"] = leaderboard.apply(lambda r: select_best(r, "best_refined_recent_12m_pf", "best_combo_recent_12m_pf"), axis=1)
    ...
    keep_cols = [
        "strategy_type", "quality_flag", "is_trades", "oos_trades", "is_pf", "oos_pf", "recent_12m_trades", "recent_12m_pf",
        "dataset", "promotion_status", "promoted_candidates", "leader_source",
...
if __name__ == "__main__":
    ...
    for family_name in family_names:
        summary_row = run_single_family(
            strategy_type_name=family_name, dataset_path=CSV_PATH, outputs_dir=OUTPUTS_DIR,
            max_workers_sweep=MAX_WORKERS_SWEEP, max_workers_refinement=MAX_WORKERS_REFINEMENT, market_symbol=MARKET_SYMBOL 
        )
    ...
    if not leaderboard_df.empty:
        ...
        print("\n🏆 LEADERBOARD 2.0 (Saved to family_leaderboard_results.csv)")
        
        # Print the exact columns ChatGPT requested instantly
        preview_lead = ["strategy_type", "quality_flag", "is_trades", "oos_trades", "is_pf", "oos_pf", "recent_12m_trades", "recent_12m_pf"]
        print(leaderboard_df[[c for c in preview_lead if c in leaderboard_df.columns]].to_string(index=False))

        print("\n" + "=" * 72 + "\n🔬 STARTING AUTOMATED PORTFOLIO EVALUATION\n" + "=" * 72)
        review_table, returns_df, corr_matrix, yearly_df = evaluate_portfolio(
            leaderboard_csv=leaderboard_path, data_csv=CSV_PATH, market_name=MARKET_SYMBOL, timeframe=TIMEFRAME,
        )
        ...
        if not review_table.empty:
            ...
            print(f"   Saved Yearly Breakdown to: {OUTPUTS_DIR / 'yearly_stats_breakdown.csv'}")
            
            preview_cols = [
                "strategy_family", "is_trades", "oos_trades",
