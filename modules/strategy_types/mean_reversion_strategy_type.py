...
class _InlineMeanReversionStrategy:
    direction = "LONG_ONLY"
    def __init__(self, filters: list[BaseFilter], hold_bars: int = 4, stop_distance_points: float = 2.0, name: str | None = None):
...
class MeanReversionStrategyType(BaseStrategyType):
    ...
    default_hold_bars, default_stop_distance_points = 4, 2.0
    ...
    def get_promotion_thresholds(self) -> dict[str, float | bool]: return {"min_profit_factor": 1.00, "min_average_trade": 0.0, "require_positive_net_pnl": False, "min_trades": 100, "min_trades_per_year": 5.0}
    ...
    def get_active_refinement_grid_for_combo(self, classes: list[type]) -> dict[str, list]:
        ...
        grid = {"hold_bars": [2, 3, 4, 5, 6, 8], "stop_distance_points": [1.0, 1.5, 2.0, 2.5]}
        grid["min_avg_range"] = [0.5, 1.0, 1.5, 2.0] if any(cls in [DistanceBelowSMAFilter, LowVolatilityRegimeFilter] for cls in classes) else [0.0]
