...
class _InlineBreakoutStrategy:
    direction = "LONG_ONLY"
    def __init__(self, filters: list[BaseFilter], hold_bars: int = 6, stop_distance_points: float = 12.0, name: str | None = None):
...
class BreakoutStrategyType(BaseStrategyType):
    name = "breakout"
    min_filters_per_combo = 3 # LOWERED TO 3 TO FIND MORE BASE COMBOS
    ...
    def build_candidate_specific_strategy(self, promoted_combo_classes: list[type], hold_bars: int, stop_distance_points: float, min_avg_range: float, momentum_lookback: int) -> _InlineBreakoutStrategy:
        filters: list[BaseFilter] = []
        for cls in promoted_combo_classes:
            if cls is CompressionFilter: filters.append(CompressionFilter(max_avg_range=min_avg_range if min_avg_range > 0 else 15.0))
            else: filters.append(cls())
        return _InlineBreakoutStrategy(filters, hold_bars, stop_distance_points, name=f"RefinedBreakoutStrategy_HB{hold_bars}_STOP{stop_distance_points}_RANGE{min_avg_range}_MOM{momentum_lookback}")
    ...
    def get_active_refinement_grid_for_combo(self, promoted_combo_classes: list[type]) -> dict[str, list]:
        # MASSIVELY EXPANDED GRID FOR BREAKOUT
        grid: dict[str, list] = {"hold_bars": [2, 4, 6, 8, 10], "stop_distance_points": [10.0, 15.0, 20.0, 25.0]}
        grid["min_avg_range"] = [10.0, 15.0, 20.0, 25.0] if any(cls is CompressionFilter for cls in promoted_combo_classes) else [0.0]
    ...
    def run_family_filter_combination_sweep(self, data: pd.DataFrame, cfg: EngineConfig, max_workers: int = 10) -> pd.DataFrame:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            for idx, result in enumerate(executor.map(_run_breakout_combo_case, tasks)):
    ...
    def run_top_combo_refinement(self, data: pd.DataFrame, cfg: EngineConfig, candidate_row: dict[str, Any], output_dir: str | Path = "Outputs", max_workers: int = 10) -> pd.DataFrame:
        ...
        refiner = StrategyParameterRefiner(MasterStrategyEngine, data, _BreakoutRefinementFactory(self, promoted_combo_classes), cfg)
