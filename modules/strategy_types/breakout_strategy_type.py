...
class _InlineBreakoutStrategy:
    direction = "LONG_ONLY"
    def __init__(self, filters: list[BaseFilter], hold_bars: int = 6, stop_distance_points: float = 2.0, name: str | None = None):
...
class BreakoutStrategyType(BaseStrategyType):
    name = "breakout"
    min_filters_per_combo, max_filters_per_combo = 3, 5 
    default_hold_bars, default_stop_distance_points = 6, 2.0
    ...
    def build_default_sanity_filters(self) -> list[BaseFilter]: return [CompressionFilter(), RangeBreakoutFilter(), ExpansionBarFilter(), BreakoutTrendFilter()]
    def build_default_strategy(self) -> _InlineBreakoutStrategy: return _InlineBreakoutStrategy(self.build_default_sanity_filters(), self.default_hold_bars, self.default_stop_distance_points, "FilterBasedBreakoutStrategy")
    def build_sanity_check_strategy(self) -> _InlineBreakoutStrategy: return self.build_default_strategy()
    def get_filter_classes(self) -> list[type]: return [CompressionFilter, RangeBreakoutFilter, ExpansionBarFilter, BreakoutRetestFilter, BreakoutTrendFilter, BreakoutCloseStrengthFilter, PriorRangePositionFilter]
    def build_filter_objects_from_classes(self, combo_classes: list[type]) -> list[BaseFilter]: return [cls() for cls in combo_classes]
    def build_combinable_strategy(self, filters: list[BaseFilter], hold_bars: int, stop_distance_points: float) -> _InlineBreakoutStrategy: return _InlineBreakoutStrategy(filters, hold_bars, stop_distance_points)
    ...
    # LOOSENED GATES TO CATCH REGIME DEPENDENT OOS WINNERS
    ...
    def get_active_refinement_grid_for_combo(self, classes: list[type]) -> dict[str, list]:
        grid = {"hold_bars": [2, 4, 6, 8, 12], "stop_distance_points": [1.0, 1.5, 2.0, 3.0]}
        grid["min_avg_range"] = [0.6, 0.7, 0.8, 0.9] if any(cls is CompressionFilter for cls in classes) else [0.0]
