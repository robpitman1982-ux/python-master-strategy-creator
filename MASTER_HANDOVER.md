...
Current status: Codex has now closed the loop between the canonical local-cluster publish path and the Google Drive cold backup, and then cleaned up the leaderboard naming ambiguity that surfaced during that mirror. The validated CFD run `2026-04-30_es_nq_validation` is finalized on c240 under `/data/sweep_results/runs/2026-04-30_es_nq_validation`; the canonical latest exports now use `master_leaderboard.csv`, `master_leaderboard_cfd.csv`, `ultimate_leaderboard_FUTURES.csv`, legacy alias `ultimate_leaderboard.csv`, and `ultimate_leaderboard_cfd.csv` under `/data/sweep_results/exports/`; and those files plus the full latest run folder have been mirrored into `G:\My Drive\strategy-data-backup\` on Latitude. Critical preservation note: the historical futures corpus was not lost when the Drive mirror refreshed `ultimate_leaderboard.csv` with the new canonical CFD-era export; the older long-lived futures leaderboard still exists locally at `Outputs/ultimate_leaderboard.csv` (649 rows, 2026-04-04 vintage) and has been restored into Drive as `ultimate_leaderboard_LEGACY_FUTURES_649rows_2026-04-04.csv`, alongside `ultimate_leaderboard_bootcamp.csv`. Important disaster-recovery note: `ultimate_leaderboard*.csv` alone is not enough for trustworthy recovery or provenance; the backup must keep the matching `sweep_results/runs/<run_id>/` folders with `cluster_run_manifest.json`, per-host logs, and dataset output artifacts. Market data is still deliberately excluded from the Google Drive cold backup, so a total server loss would preserve published leaderboard state and run provenance, but not the full raw research corpus.
