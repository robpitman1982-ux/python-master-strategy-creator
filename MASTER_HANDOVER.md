...
Current status: Codex bulletproofed the CFD sweep path, validated the whole cluster, and then removed the old Bootcamp-shaped ranking from the CFD research pipeline. Instrument-universe guardrails separate `futures_tradestation` from `cfd_dukascopy`, preflight rejects futures-vs-CFD data/economics mismatches, and CFD configs use canonical `{MARKET}_{TF}_dukascopy.csv` names with corrected The5ers-mapped CFD economics for ES/NQ/YM/GC/SI/CL. `run_cluster_sweep.py --jobs MARKET:TF ...` runs exact assignments without creating a market/timeframe cross-product; `run_distributed_sweep.py` partitions jobs across hosts by worker capacity and validates every generated config before printing per-host commands. Cluster validation completed: all four hosts have the staged tree at `/tmp/psc_ad8bf7e_full`, all four have 120 CFD OHLC files at `/data/market_data/cfds/ohlc_engine`, and g9 now has `requirements.txt` installed in `~/venv`. Near-max 60m stress test reached about c240 90%, gen8 92%, r630 91%, g9 58% CPU, then was stopped deliberately. Daily completion test passed on all hosts: c240 CL daily DONE (14 accepted), gen8 NQ daily DONE (15 accepted), r630 GC daily DONE (14 accepted), g9 BP daily DONE (13 accepted), each with 34 output files and ~35-36 MB artifacts. CFD family/master/ultimate outputs are now neutral (`family_leaderboard_results.csv`, `master_leaderboard_cfd.csv`, `ultimate_leaderboard_cfd.csv`), `bootcamp_score` is no longer emitted into CFD leaderboards, and the selector now owns program-specific ranking using neutral prefilter priority plus the existing MC/evaluation pipeline. Futures Bootcamp-ranked outputs remain intact for the futures research track. Local verification: `python -m pytest tests/ --ignore=tests/test_engine_parity.py -q` -> 303 passed, 1 known date-parse warning. **Next session starts here:** decide whether to push the new local commits, then regenerate CFD aggregates from finished run artifacts or launch the first real CFD sweep batch using recommended workers c240:72, gen8:44, r630:80, g9:28-34. Cluster is currently quiet; no sweep processes running. Prior Sessions 76-83 plus prop firm audit remain complete.
...
- **CFD leaderboard architecture (2026-04-30):** canonical CFD strategy pool is now neutral. CFD runs write `family_leaderboard_results.csv`, `master_leaderboard_cfd.csv`, and `ultimate_leaderboard_cfd.csv`; `bootcamp_score` and `*_bootcamp.csv` ranking views are no longer emitted for CFD runs. Futures runs still keep the older Bootcamp-ranked views unchanged.
- **Portfolio selector:** 6-stage pipeline with 3-layer correlation, block bootstrap MC, regime survival gate. Candidate gating no longer uses `bootcamp_score_min`; selector reads the neutral CFD pool and does program-specific ranking inside the selector/MC layer.
