...
Current status: Sessions 76-83 plus prop firm audit complete (2026-04-30). Operator briefly switching to Codex while Claude credits reload — Claude returns tomorrow evening (2026-05-01). HANDOVER_FOR_CODEX.md at repo root provides full cold-start brief for Codex. Sessions 76-83 shipped six discipline upgrades (BH-FDR gate, DSR on leaderboard, walk-forward module, random-flip null test, EXTERNAL_LLM_BRIEFING, sprint architecture). Today's prop firm audit added: full website verification of all three target programs (Bootcamp, High Stakes, Pro Growth), six factory + simulator bugs patched (sequential daily-DD recalc formula, vectorized daily-DD recalc, Bootcamp track-specific step balances, Bootcamp $20K fees, Pro Growth $10K fee, High Stakes full fee_map for 6 tracks), and a canonical `docs/the5ers_program_rules.md`. 287/287 tests pass. **Next session starts here:** check Codex's commits since `dac9036`, then assess whether to start CFD swap-cost integration (Open Issue #8) or pre-register Sprint 84 (ES sanity-check sweep with new gates enabled). (1) **BH-FDR family-aware promotion gate** in `modules/statistics.py` + wired into `apply_promotion_gate`. Opt-in via `promotion_gate.bh_fdr_alpha`. (2) **Deflated Sharpe Ratio** on master leaderboard, per-leader trial count from sweep CSV row counts. New columns: `deflated_sharpe_ratio`, `sharpe_per_trade`, `n_trials_in_search`. (3) **Walk-forward validation module** `modules/walk_forward.py` — rolling 3yr-train/1yr-test windows, mean+min t-stat, opt-in gate. (4) **Random-flip null permutation test** (n=5000, vectorised) in `modules/statistics.py`, robust z-statistic against random-direction null. (5) **EXTERNAL_LLM_BRIEFING.md** for cross-LLM consultations (ChatGPT/Gemini). (6) **Sprint architecture** under `sessions/` — pre-registered specs with frozen grids and verdict semantics (NO EDGE / CANDIDATES / SUSPICIOUS / BLOCKED). **Throughput finding from Session 76:** `run_cluster_sweep.py` is single-host sequential — 3/4 cluster hosts idle during sweeps. Refactor needed before next 24-market sweep, deferred to clean cluster window. **Deferred items:** g9 onboarding + throughput refactor (need clean cluster window). **Earlier this week:** Session 75 (2026-04-26): Hermes + OpenClaw fully decommissioned. Server-side: users deleted, /data/hermes + /data/openclaw + /data/shared wiped (~1.5 GB), cron + scripts + sudoers removed, Hermes SSH pubkey purged from c240/gen8/r630 authorized_keys (backups taken). External: GitHub deploy keys revoked (both repos), Telegram bot deleted, Hermes Anthropic API key revoked. Hermes is fully gone, no remaining state. g9 is now a compute worker (48 threads, 32 GB RAM) like the rest of the cluster — repo clone + market_data + sweep scripts not yet set up. Earlier in session: `HANDOVER.md` renamed to `MASTER_HANDOVER.md` for project-scoped naming; GCS buckets deleted manually by operator (cloud infrastructure decommissioned). Prior Session 73/72h: ALL-SERVERS-ALWAYS-ON policy is HARD RULE; g9 back online via SAS reseat + cable verify. Prior Session 74: X1 Carbon runs parallel Claude Remote Control hub for python-master-strategy-creator (ops at `C:\Users\rob_p\strategy-ops\`).
