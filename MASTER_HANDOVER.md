...
Current status: Codex added instrument-universe guardrails before CFD sweeps and proved the first c240 gate. `modules/instrument_universe.py` now defines `futures_tradestation` vs `cfd_dukascopy`, validates dataset source, canonical CFD basenames, and known futures/CFD contract economics. `run_local_sweep.py`, `run_cluster_sweep.py`, and direct `master_strategy_engine.py --config ...` all fail closed on universe mismatches. CFD configs were regenerated with explicit `instrument_universe: cfd_dukascopy`, canonical no-year Dukascopy basenames, and corrected The5ers-mapped CFD economics for ES/NQ/YM/GC/SI/CL. Verification: `python -m pytest tests/ --ignore=tests/test_engine_parity.py -q` -> 295 passed, 1 known date-parse warning; local 96-job CFD config dry-run validates; c240 isolated-tree ES daily CFD dry-run and execution both completed using `/data/market_data/cfds/ohlc_engine/ES_daily_dukascopy.csv`, `instrument_universe: cfd_dukascopy`, `tick_value: 0.01`, `dollars_per_point: 1.0`. ES daily output: 15/15 families DONE in ~423s, 13 accepted final, 37 MB artifacts at `/tmp/psc_0e525e4/Outputs/es_daily_dukascopy_v1/ES_daily`. **Next session starts here:** decide how to sync commit `0e525e4` plus this follow-up doc commit to c240/workers, then run a multi-timeframe or multi-host dry run; do not start full cluster compute until worker repos are aligned and the cluster dispatcher/g9 decision is made. Prior Sessions 76-83 plus prop firm audit remain complete: BH-FDR gate, DSR, walk-forward, random-flip null, sprint architecture, and `docs/the5ers_program_rules.md`.
