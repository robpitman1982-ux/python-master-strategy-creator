...
Current status: Sessions 76-78 partial (2026-04-30): operator switched dev machine to Latitude (Claude Code in VS Code) and consulted betfair Claude for cross-project review. Three of four planned items executed under Option C (no cluster work — betfair Claude is using the cluster for sweeps). (1) **Throughput investigation:** confirmed `run_cluster_sweep.py` is misleadingly named — it's a single-host sequential batch runner; 3/4 cluster hosts idle during sweeps. Refactor needed, deferred to clean cluster window. (2) **BH-FDR family-aware promotion gate** added (`modules/statistics.py` + wired into `apply_promotion_gate`); opt-in via `promotion_gate.bh_fdr_alpha` config. (3) **Deflated Sharpe Ratio** added to master leaderboard (Bailey & Lopez de Prado 2014); per-leader trial count sourced from sweep CSV row counts. New columns on `master_leaderboard.csv`: `deflated_sharpe_ratio`, `sharpe_per_trade`, `n_trials_in_search`. 42 new tests, full suite 257/257 pass. **Deferred items:** g9 onboarding + throughput refactor (need clean cluster window). **Earlier this week:** Session 75 (2026-04-26): Hermes + OpenClaw fully decommissioned. Server-side: users deleted, /data/hermes + /data/openclaw + /data/shared wiped (~1.5 GB), cron + scripts + sudoers removed, Hermes SSH pubkey purged from c240/gen8/r630 authorized_keys (backups taken). External: GitHub deploy keys revoked (both repos), Telegram bot deleted, Hermes Anthropic API key revoked. Hermes is fully gone, no remaining state. g9 is now a compute worker (48 threads, 32 GB RAM) like the rest of the cluster — repo clone + market_data + sweep scripts not yet set up. Earlier in session: `HANDOVER.md` renamed to `MASTER_HANDOVER.md` for project-scoped naming; GCS buckets deleted manually by operator (cloud infrastructure decommissioned). Prior Session 73/72h: ALL-SERVERS-ALWAYS-ON policy is HARD RULE; g9 back online via SAS reseat + cable verify. Prior Session 74: X1 Carbon runs parallel Claude Remote Control hub for python-master-strategy-creator (ops at `C:\Users\rob_p\strategy-ops\`).
