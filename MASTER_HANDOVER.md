...
Current status: Codex added instrument-universe guardrails before CFD sweeps, proved the first c240 gate, and added exact-job distributed planning. `modules/instrument_universe.py` now defines `futures_tradestation` vs `cfd_dukascopy`, validates dataset source, canonical CFD basenames, and known futures/CFD contract economics. `run_local_sweep.py`, `run_cluster_sweep.py`, and direct `master_strategy_engine.py --config ...` all fail closed on universe mismatches. `run_cluster_sweep.py --jobs MARKET:TF ...` now runs exact assignments without creating a market/timeframe cross-product. New `run_distributed_sweep.py` partitions jobs across hosts by worker capacity and prints per-host exact `--jobs` commands after validating each generated config. Verification: `python -m pytest tests/ --ignore=tests/test_engine_parity.py -q` -> 302 passed, 1 known date-parse warning; distributed 96-job CFD plan validates for `c240:36 gen8:24 r630:32 g9:16`; c240 isolated-tree ES daily CFD dry-run and execution both completed using `/data/market_data/cfds/ohlc_engine/ES_daily_dukascopy.csv`, `instrument_universe: cfd_dukascopy`, `tick_value: 0.01`, `dollars_per_point: 1.0`. ES daily output: 15/15 families DONE in ~423s, 13 accepted final, 37 MB artifacts at `/tmp/psc_0e525e4/Outputs/es_daily_dukascopy_v1/ES_daily`. **Next session starts here:** sync commits to c240/workers or stage an isolated archive, then run per-host dry-run commands from `run_distributed_sweep.py --dry-run-commands`; do not start full cluster compute until worker repos are aligned and g9 readiness is confirmed. Prior Sessions 76-83 plus prop firm audit remain complete: BH-FDR gate, DSR, walk-forward, random-flip null, sprint architecture, and `docs/the5ers_program_rules.md`.
