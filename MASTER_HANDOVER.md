...
Current status: Codex added instrument-universe guardrails before CFD sweeps. `modules/instrument_universe.py` now defines `futures_tradestation` vs `cfd_dukascopy`, validates dataset source, canonical CFD basenames, and known futures/CFD contract economics. `run_local_sweep.py`, `run_cluster_sweep.py`, and direct `master_strategy_engine.py --config ...` all fail closed on universe mismatches. CFD configs were regenerated with explicit `instrument_universe: cfd_dukascopy`, canonical no-year Dukascopy basenames, and corrected The5ers-mapped CFD economics for ES/NQ/YM/GC/SI/CL. New doc: `docs/INSTRUMENT_UNIVERSES.md`. Verification: `python -m pytest tests/ --ignore=tests/test_engine_parity.py -q` -> 295 passed, 1 known date-parse warning; `run_cluster_sweep.py --timeframes daily 60m 30m 15m --data-dir /data/market_data/cfds/ohlc_engine --workers 36 --dry-run` validates 96 CFD job configs. **Next session starts here:** sync this commit to c240/workers, then run c240 ES daily dry-run/execution; do not start full cluster compute until worker repos are aligned and g9 onboarding/dispatcher decision is made. Prior Sessions 76-83 plus prop firm audit remain complete: BH-FDR gate, DSR, walk-forward, random-flip null, sprint architecture, and `docs/the5ers_program_rules.md`.
