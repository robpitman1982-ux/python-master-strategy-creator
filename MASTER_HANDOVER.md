...
Current status: Codex launched the first real multi-host CFD batch on the validated cluster at `2026-04-30T16:25:25+10:00` from local commit `9ed5648`, using fresh staged trees at `/tmp/psc_9ed5648` on all four hosts. Scope: `ES` + `NQ` only, timeframes `daily`, `60m`, `30m`, data dir `/data/market_data/cfds/ohlc_engine`, with the new neutral CFD leaderboard path intentionally empty before launch (`Outputs/master_leaderboard_cfd.csv` and `Outputs/ultimate_leaderboard_cfd.csv` absent locally). Exact assignments now running under `nohup` with per-host logs in `/tmp/psc_logs/`: c240 `ES:30m` at 72 workers, gen8 `ES:60m` at 44 workers, r630 `ES:daily NQ:daily NQ:60m` at 80 workers, g9 `NQ:30m` at 28 workers. Remote logs use UTC timestamps while the control plane uses Sydney time; initial live snapshots showed c240 `ES:30m` ETA about 861 min, gen8 `ES:60m` about 1048 min, g9 `NQ:30m` about 783 min, while r630 burned quickly through the daily work before reaching `NQ:60m`. The pipeline remains bulletproofed and neutral: instrument-universe guardrails separate `futures_tradestation` from `cfd_dukascopy`, no new Bootcamp-ranked outputs are emitted, futures use canonical `family_leaderboard_results.csv` / `master_leaderboard.csv` / `ultimate_leaderboard.csv`, CFDs use `family_leaderboard_results.csv` / `master_leaderboard_cfd.csv` / `ultimate_leaderboard_cfd.csv`, and the selector owns program-specific ranking downstream. Local verification before launch remained `python -m pytest tests/ --ignore=tests/test_engine_parity.py -q` -> 303 passed, 1 known date-parse warning. **Next session starts here:** monitor `/tmp/psc_logs/psc_9ed5648_es_nq_{c240,gen8,r630,g9}.log`, confirm the six assigned jobs finish cleanly, then inspect what populated into the new CFD aggregates and record wall-clock timings for the three timeframe tiers. Cluster is busy with the in-flight batch.
